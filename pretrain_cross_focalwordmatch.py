...
save_dir = 'output/cross_pretrain/focal_word_match/5l1l2exp/'
...
class focal_loss(nn.Module):
    ...
    def forward(self, preds, labels): # 传入的都为一维向量
        preds_sigmoid = torch.sigmoid(preds)
        preds_sigmoid = labels * preds_sigmoid + (1-labels) * (1-preds_sigmoid)
        preds_log = torch.log(preds_sigmoid)

        loss = -torch.mul(torch.pow((1-preds_sigmoid), self.gamma), preds_log) 
        loss = loss.mean()
