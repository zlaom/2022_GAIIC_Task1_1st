...
class FuseModel(nn.Module):
    def __init__(self, split_config, fuse_config, vocab_file,img_p=0.2, img_dim=2048, n_img_expand=8):
        ...
        self.image_encoder = ImageModel(img_dim=img_dim, hidden_size=fuse_config.hidden_size, p=img_p, n_img_expand=n_img_expand)
...
class FuseModel2TasksNewDropout(nn.Module):
    def __init__(self, split_config, fuse_config, vocab_file,img_p=0.2, img_dim=2048, n_img_expand=8):
        ...
        self.head = nn.Linear(fuse_config.hidden_size * 2 , 2)
        # self.head = nn.Linear(fuse_config.hidden_size*2, 2)
        self.word_head = nn.Linear(fuse_config.hidden_size , 2)
    ...
    def forward(self, image_features, splits, word_match=False): # 注意splits需要为二维列表
        ...
        if word_match:
            split_embedding = fuse_output[:, self.n_img_expand+1:, :]
            split_embedding = self.word_head(split_embedding)
            return x, split_embedding, attention_mask
        return x
