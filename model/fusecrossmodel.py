...
class FuseCrossModelWithFusehead(nn.Module):
    def __init__(self, split_config, fuse_config, cross_config, vocab_file, img_dim=2048, n_img_expand=8):
        super().__init__()
        self.n_img_expand = n_img_expand
        ...
        self.head = nn.Linear(fuse_config.hidden_size, 1)
        self.word_head = nn.Linear(cross_config.hidden_size, 1)
        self.fuse_head = nn.Linear(cross_config.hidden_size + fuse_config.hidden_size, 1)
        # self.fuse_head = nn.Sequential(
        #     nn.Linear(cross_config.hidden_size + fuse_config.hidden_size, fuse_config.hidden_size),
        #     nn.LayerNorm(fuse_config.hidden_size),
        #     nn.ReLU(inplace=True),
        #     nn.Linear(fuse_config.hidden_size, 1)
        # )
    ...
    def forward(self, image_features, splits, word_match=False): # 注意splits需要为二维列表
        ...
        # 输出头
        if word_match:
            B, W, D = split_outputs[0].shape
            cls_token = fuse_output[:, 0, :] # 这步数据会变成2维
            cls_token_expand = cls_token.unsqueeze(1).expand(B, W, D)
            fuse_outputs = torch.cat([split_outputs[0], cls_token_expand], dim=-1)
            x = self.fuse_head(fuse_outputs)
            return x, split_attention_mask
