...
from model.crossmodel import FinetuneCrossModel
...
save_dir = 'output/title_finetune/cross/focalwordmatch/5l1l1l2exp/'
...
FREEZE = False
LOAD_CKPT = True
ckpt_file = 'output/cross_pretrain/focal_word_match/5l1l2exp/0.8776.pth'
...
cls_config = BertConfig(num_hidden_layers=cls_layers)
model = FinetuneCrossModel(split_config, cross_config, cls_config, vocab_file, n_img_expand=n_img_expand)
if LOAD_CKPT:
    msg = model.load_state_dict(torch.load(ckpt_file), strict=False)
    print(msg)
model.cuda()

# freezing
if FREEZE:
    unfreeze_layers = ['cls_token', 'cls_splitbert', 'cls_head']
