...
from model.fusion_model import FusionAttrMlp, FusionTitleMlp
...
save_dir = "data/model_data/fusion/baseline/title_layer2_split_01"
...
#     with open(f"data/tmp_data/fusion/predict/merge_{split_index}.txt", "r") as f:
#         for line in tqdm(f):
...
def evaluate(model, val_dataloader, loss_fn):
    ...
    for batch in tqdm(val_dataloader):
        ...
        new_title_predict[new_title_predict > threshold] = 1
        new_title_predict[new_title_predict <= threshold] = 0

        correct += torch.sum(title_label == new_title_predict).cpu()
    ...
    if fold_id == args.fold_id:
        ...
        for epoch in range(max_epoch):
            ...
            for i, batch in enumerate(train_dataloader):
                ...
                correct += torch.sum(new_title_predict == title_label).cpu()
                total += len(title_label)
