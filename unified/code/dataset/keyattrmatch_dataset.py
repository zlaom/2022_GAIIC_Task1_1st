...
class AttrIdMatchDataset(Dataset):
    ...
    def __init__(
        self, input_filename, neg_attr_dict_file, attr_to_id_file, random_neg=False
    ):
                        ...
                        if item["key_attr"]:  # 必须有属性
                            ...
                            for attr_key, attr_value in item["key_attr"].items():
                                ...
                                if not random_neg:
                                    ...
                                    new_item["key"] = attr_key
                                    new_item["label"] = 0
                                    sample_attr_list = self.neg_attr_dict[attr_value][
                                        "similar_attr"
                                    ]
...
# attr id match dataset
class AttrIdDistinguishDataset(Dataset):
    """generate positive and negative samples for attribute matching finetuning"""
                        ...
                        if item["key_attr"]:  # 必须有属性
                            ...
                            for attr_key, attr_value in item["key_attr"].items():
                                ...
                                if attr_key == key_attr:
                                    ...
                                    i += 1
                                    if not random_neg:
                                        sample_attr_list = self.neg_attr_dict[
                                            attr_value
                                        ]["similar_attr"]
                                        attr_value = random.sample(
                                            sample_attr_list, k=1
                                        )[0]
                                        new_item["attr"] = attr_value
                                        new_item["label"] = 0
                                    i += 1
...
class AttrMatchProbaDataset(Dataset):
    ...
    def __init__(self, input_filename, attr_dict_file, vocab_dict_file):
        ...
        self.negative_dict, self.proba_negative_dict = self.get_negative_dict(
            attr_dict, vocab_dict
        )
        # 提取数据
        self.items = []
        for file in input_filename.split(","):
    ...
    def __getitem__(self, idx):
        ...
        for query, attr in key_attr.items():
            ...
            attr_mask[attr_index] = 1
            if random.random() > 0.5:
                split[attr_index] = np.random.choice(
                    self.negative_dict[attr], p=self.proba_negative_dict[attr]
                )
                split_label[attr_index] = 0  # 标签不匹配
...
def attr_id_distinguish_collate_fn(batch):
    images = []
    attr_ids_list = []
    labels = []
