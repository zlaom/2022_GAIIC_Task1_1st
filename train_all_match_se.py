...
def get_dataloader(dataset_cfg):
    ...
    train_dataset = FuseReplaceDataset(train_path, attr_dict_path, vocab_dict, is_train=True)
    val_dataset = FuseReplaceDataset(val_path, attr_dict_path, vocab_dict, is_train=False)
    ...
    for epoch in range(dataset_cfg['EPOCHS']):
        ...
        if acc > best_acc:
            ...
            torch.save(model.state_dict(),
                    os.path.join(output_folder, 'se_split_43_h6_best_acc.pth'))
        if best_loss > val_loss:
            best_loss = val_loss
            torch.save(model.state_dict(),
                    os.path.join(output_folder, 'se_split_43_h6_best_loss.pth'))
