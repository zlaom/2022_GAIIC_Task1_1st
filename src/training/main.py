...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    if args.precision == "amp" or args.precision == "fp32" or args.gpu is None:
        convert_models_to_fp32(model) # 又转回了32
    ...
    scaler = GradScaler() if args.precision == "amp" else None # 放大梯度 缩梯度
    # optionally resume from a checkpoint
    start_epoch = 0
    if args.resume is not None: #false
        if os.path.isfile(args.resume):
            if args.gpu is None:
                checkpoint = torch.load(args.resume)
            else:
                # Map model to be loaded to specified single gpu.
                loc = "cuda:{}".format(args.gpu) # gpu 2?
                checkpoint = torch.load(args.resume, map_location=loc)
            start_epoch = checkpoint["epoch"]
            sd = checkpoint["state_dict"]
            if not args.distributed and next(iter(sd.items()))[0].startswith('module'): # TODO understand
    ...
    writer = None
    if args.save_logs and args.tensorboard: # false
        writer = SummaryWriter(args.tensorboard_path)

    if args.wandb and is_master(args): #false
    ...
    if args.train_data is None: #false
        evaluate(model, data, start_epoch, args, writer, 0)
        return
    elif start_epoch == 0 and args.val_data is not None: #true
...
def main():
    ...
    args.wandb = 'wandb' in args.report_to or 'all' in args.report_to # False
    args.tensorboard = 'tensorboard' in args.report_to or 'all' in args.report_to # False
    ...
    args.distributed = (args.gpu is None) and torch.cuda.is_available() and (not args.dp) # false
    if args.distributed:
