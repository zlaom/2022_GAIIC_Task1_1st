            ...
            if self.items[idx]['key_attr']: # 属性存在才可能进行替换
                for query, attr in self.items[idx]['key_attr'].items():
                    if random.random() > 0.5: # 负例生成
                        new_attr = random.sample(self.negative_dict[f'{query}-{attr}'], 1)[0]
...
# data
# train_file ='../data/preprocessed_data/unit_train_fine.txt.00,../data/preprocessed_data/unit_coarse_fine.txt'
...
# val_file = '../data/preprocessed_data/unit_train_fine.txt.01,../data/preprocessed_data/unit_neg_coarse_fine.txt'
