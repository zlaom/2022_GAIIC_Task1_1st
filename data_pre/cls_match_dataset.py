...
class FuseReplaceAttrDataset(Dataset):
    def __init__(self, input_filename, attr_dict_file, vocab_dict, is_train):
        self.is_train = is_train
        with open(attr_dict_file, 'r') as f:
            attr_dict = json.load(f)
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            split = copy.deepcopy(list(item['key_attr'].values())) # 要做拷贝，否则会改变self.items的值
            label = 1
            if random.random() < 0.5:
                new_index = np.random.randint(len(split))
                split[new_index] = np.random.choice(self.negative_dict[split[new_index]])
                label = 0
            else:
                label = 1
        else:
