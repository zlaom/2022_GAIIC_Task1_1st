                ...
                for i in rep_idx:
                    ...
                    if word in self.negative_dict: # 如果是关键属性则属性替换
                        ...
                        if flag:
                            ...
                            split[i] = new_word
                split_index[rep_idx[0]] = 1
                split_lable = 0
            else:
                split_index[random.sample([i for i in range(len(split))], 1)[0]] = 1
                split_lable = 1
                label = 1
            return image, split, label, split_index, split_lable
        else:
            label = item['match']['图文']
...
def cls_collate_fn(batch):
    ...
    split_labels = []
    split_indexes = []
    for feature, split, label, split_index, split_label in batch:
    ...
    split_indexes = torch.stack(split_indexes)
    return tensors, splits, labels, split_indexes, split_labels

def cls_collate_fn_finetune(batch):
    tensors = []
    splits = []
    labels = []
